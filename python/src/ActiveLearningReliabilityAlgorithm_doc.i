...
:math:`\forall \mathbf{x} \in \mathbb{R}^{\inputDim}, \mathbf{1}_{\model(\mathbf{x})<T} = \mathbf{1}_{\hat{\model}(\mathbf{x})<T}` with :math:`\hat{\model}` the surrogate model of :math:`g`. 
